...
```{r load and clean data2, echo=FALSE, warning=TRUE, results='hide', message=FALSE}
# Percent democrat
...
```{r plot data1, echo=FALSE, warning=TRUE, message=FALSE}
library(magrittr)
...
```{r plot data2, echo=FALSE, warning=TRUE, message=FALSE}
figure2 %<>%
...
```{r load and clean data6, echo=TRUE, warning=TRUE, message=FALSE, results='hide'}
num_equal <- as.integer(as.logical(jk$kamala == jk$joe)) # find observations with no sign
...
binom.test(sum(kamala_greater), length(kamala_greater) - sum(num_equal), p = 0.5,
           ...
           conf.level = 0.95)
```
```{r equal stats, echo=FALSE, warning=TRUE, message=FALSE, results='hide'}
num_equal <- as.integer(as.logical(jk$kamala == jk$joe))
equal_percentage <- sum(num_equal) / length(num_equal) * 100
print(paste("Percentage of equal feeling score: ", round(equal_percentage, 2),"%"))
print(paste('Number of equal paired scores:', sum(num_equal)))
```



I fail to find evidence that there is a difference in enthusiasm for Joe Biden and Kamala Harris in Democratic voters. The p-value for the sign test is 0.0742. Generally speaking, 2483 data points is enough data to use a sign test in a significant manner. However, because 41.56% (1032 observations) of the sample recorded an equal level of enthusiasm for both politicians, these data points resulted in no sign in the sign test, essentially ruling them out. This percentage is not surprising given the similar distributions visible in the histograms and the clear correlation between points visible in the density scatter plot. Should a larger amount of points been used that did not have an equal score for both Biden and Harris, this may have had a different outcome (ex. if this test is run with twice the amount of data of the same ratio, the p-value comes out as 0.011).

Although the test is negative, we briefly consider the effect size. On top of the large share of Democratic voters that share equal enthusiasm between Joe Biden and Kamala Harris, 27.83% of the observations indicate a greater feeling thermometer score for Kamala Harris than Joe Biden and 30.61% indicate the inverse. In ruling out the observations of equal opinion, this results in 47.62% of the voters having greater enthusiasm for Kamala Harris than Joe Biden, and 52.38% for the inverse. These results do not indicate a strong polarization for either party member. Given that Harris stands on fairly equal ground with Biden in this regard, it is a further testimony for her ability to excel as a politician in the future and lead the Democratic party and country as Biden is today. Furthermore, it is notable that while Biden and Harris have many readily apparent differences, Democratic voters across the nation appear to see them with equal enthusiasm. This is promising for the party as Harris is just 57 years old, young in the years of a national politician.





```{r example test, echo=FALSE, warning=TRUE, message=FALSE, results='hide'}
library(BSDA)
new_joe <- rbind(jk$joe, jk$joe)
new_kamala <- rbind(jk$kamala, jk$kamala)
